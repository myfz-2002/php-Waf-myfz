...
require_once('/www/wwwroot/myfz.tk/360/black/blacklist.php');
//防护脚本版本号
...
require_once("/www/wwwroot/myfz.tk/360/send_email.php");
 if( mailk($logs)){
 }
...
$currentUrl = $protocol . '://' . $host . $uri;
  ...
  if (preg_match("/".$ArrFiltReq."/is",$StrFiltValue)==1){
    ...
    webscan_slog(json_encode(array('cont' => array('ip' => getRealIp(),'time'=>strftime("%Y-%m-%d %H:%M:%S"),'Host'=>$currentUrl,'page'=>$_SERVER["PHP_SELF"],'method'=>$method,'rkey'=>$StrFiltKey,'rdata'=>$StrFiltValue,'user_agent'=>$_SERVER['HTTP_USER_AGENT'],'request_url'=>$_SERVER["REQUEST_URI"]))),480);
    addToBlacklist(getRealIp());
    exit(webscan_pape());
  }
  if (preg_match("/".$ArrFiltReq."/is",$StrFiltKey)==1){
   ...
   webscan_slog(json_encode(array('cont' => array('ip' => getRealIp(),'time'=>strftime("%Y-%m-%d %H:%M:%S"),'Host'=>$currentUrl,'page'=>$_SERVER["PHP_SELF"],'method'=>$method,'rkey'=>$StrFiltKey,'rdata'=>$StrFiltKey,'user_agent'=>$_SERVER['HTTP_USER_AGENT'],'request_url'=>$_SERVER["REQUEST_URI"]))),480);
   addToBlacklist(getRealIp());
    exit(webscan_pape());
  }
